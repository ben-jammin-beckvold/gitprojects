...
VL53L0X::VL53L0X(const int8_t expanderAddr, const uint8_t address) {
    this->expanderAddr = expanderAddr;
...
void VL53L0X::initIOexpander() {
    // Use IO expander to set the xshut pin off
    // address set in 
    if (this->expanderAddr >= 0) {
        // set expander pins to OUTPUT
        bool p = I2Cdev::writeByte(this->expanderAddr, IOEX_OUTPUT_REG, ALL_OUTPUT);
        if (!p) {
            throw(std::string("Error writing ALL_OUTPUT to register: ") + std::string(strerror(errno)));
        }
        // set expander pins to OFF
        bool p = I2Cdev::writeByte(this->expanderAddr, IOEX_OUTPUT_REG, ALL_LOW);
        if (!p) {
            throw(std::string("Error writing ALL_LOW to register: ") + std::string(strerror(errno)));
        }

        // VL53 ranger should be off until the power_on function call
    }
...
void VL53L0X::init(bool ioMode2v8) {
    ...
    this->writeRegister(SYSTEM_INTERRUPT_CONFIG_GPIO, 0x00);
    // active low
...
void VL53L0X::powerOn() {
    ...
    if (this->expanderAddr >= 0) {
        // set expander pins to ON
        bool p = I2Cdev::writeByte(this->expanderAddr, IOEX_OUTPUT_REG, ALL_HIGH);
        if (!p) {
            throw(std::string("Error writing ALL_LOW to register: ") + std::string(strerror(errno)));
        }
